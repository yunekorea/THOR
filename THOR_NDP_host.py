...
def compute_struct_size(ct: DataStruct) -> int:
    total_bytes = 0
    for tensor in ct.data:
        # nelement() is the number of coefficients
        # element_size() is the bytes per coefficient (e.g., 8 bytes for float64/int64)
...
log_mem("after pk/evk/gk/conjk loaded")
...
key_timer.stop()
print("DONE")
log_mem("after key init (key_timer stopped)")

# ----------------------------------------------------------------------
# Instrument every bootstrap() call. On the NDP host, CkksNDPEngine's
# .bootstrap() may run locally or transparently round-trip to the Target
# over RDMA -- either way, this wrapper logs memory immediately before
# and after each call without needing to know which path was taken.
# ----------------------------------------------------------------------
...
for layer_idx in range(12):
    ...
    # Same diagnostic as THOR_baseline.py: 'variables' is never read after
    # this point, but stays alive in GPU memory until the *next* call to
    # forward_layer() reassigns it -- freeing it explicitly here shows
